...
describe("BillingService", () => {
  ...
  describe("getBillingInfo", () => {
    ...
    it("should return portal URL when polar customer exists and env is set", async () => {
      process.env.POLAR_PORTAL_URL = "https://polar.sh/test/portal";
      mockPrisma.subscription.upsert.mockResolvedValue({
        id: "sub-1",
        centerId,
        status: "active",
      ...
      const result = await service.getBillingInfo(centerId);
      expect(result.portalUrl).toBe("https://polar.sh/test/portal");

      delete process.env.POLAR_PORTAL_URL;
    });
