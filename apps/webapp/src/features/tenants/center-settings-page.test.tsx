...
vi.mock("react-hook-form", () => {
  ...
  function useForm(props: any) {
    ...
    return {
      ...
      handleSubmit: (fn: any) => (e: any) => { e?.preventDefault?.(); fn(valuesRef.current); },
      formState: { isSubmitting: false, errors: {} },
      reset: (v: any) => { valuesRef.current = v ?? defaults; },
  ...
  function Controller({ render: renderProp, name, control }: any) {
    ...
    return renderProp({
      field: { value, onChange: setValue, onBlur: () => {}, name, ref: () => {} },
...
vi.mock("@workspace/ui/components/form", () => {
  ...
  const FormField = ({ render: renderProp, name, control }: any) => {
    ...
    return renderProp({
      field: {
        value,
        onChange: (v: any) => setValue(typeof v?.target !== "undefined" ? v.target.value : v),
        onBlur: () => {},
        name,
        ref: () => {},
      },
...
vi.mock("@workspace/ui/components/popover", () => {
  ...
  const Popover = ({ children, open, onOpenChange }: any) => {
    ...
    return React.createElement(Ctx.Provider, { value: { open: open ?? isOpen, onOpenChange: onOpenChange ?? setIsOpen } }, children);
  };
...
vi.mock("@workspace/ui/components/command", () => {
  ...
  const CommandList = ({ children }: any) => React.createElement("div", null, children);
  const CommandEmpty = ({ children }: any) => {
    const filter = React.useContext(FilterCtx);
    if (!filter) return null;
    return React.createElement("div", null, children);
  };
  const CommandGroup = ({ children }: any) => React.createElement("div", null, children);
